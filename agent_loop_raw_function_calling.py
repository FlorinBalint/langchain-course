...
def apply_discount(price: float, discount_tier: str) -> float:
    """
    Apply a discout to a price and return the final price
    Available tires: bronze, silver, gold
    """

    price = float(price)
    print(
        f"    >>> Executin apply_discount(price={price}, discount_tier={discount_tier})"
    )
...
def run_agent(question: str):
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "2. Only call apply_discount AFTER you received a price from"
                "get_product_price first.\n\n"
                "Pass the exact price returned by the get_product_price - do NOT pass a made-up number.\n"
                "3. NEVER calculate discounts yourself using math. "
                "Always use the apply_discount tool.\n"
                "4. If the user does not specify a discount tier, do not assume one."
                "Ask the user which tier to use instead.\n\n"
            ),
        },
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        # If no tool calls, this is the answer
        if not tool_calls:
        ...
        print(f"Tool {tool_name} selected with args {tool_args} ")
...
if __name__ == "__main__":
    result = run_agent(
        "What is the price of a keyboard after applying silver discount?"
    )
